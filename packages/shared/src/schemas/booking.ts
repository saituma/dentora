...
const BookingRulesBaseSchema = z.object({
  id: z.string().uuid(),
  ...
  emergencySlotPolicy: z.record(z.string(), z.unknown()),
  rescheduleLimits: z.record(z.string(), z.unknown()).nullable().default(null),
  afterHoursPolicy: z.record(z.string(), z.unknown()),
  validationState: z.enum(['valid', 'warning', 'blocked']),
  createdAt: z.coerce.date(),
  updatedAt: z.coerce.date(),
});

export const BookingRulesSchema = BookingRulesBaseSchema.refine(
...
export const CreateBookingRulesSchema = BookingRulesBaseSchema.omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
