# CS R4+ Integration Foundation

Status: foundation only. Dentora does not currently make production CS R4+ API calls.
...
- Feature flag: `ENABLE_CS_R4_PLUS=false` by default.
- Adapter path: `apps/server/src/modules/pms/adapters/cs-r4-plus/`.
...
CS R4+ can move from foundation-only to implementation only after official docs, sandbox/demo access, connector requirements, scheduling operations, auth, and legal/data-processing checks are approved in the PMS readiness checklist.

It can move from implementation to live scheduling only after the production adapter exists, sandbox contract tests pass against CS R4+, and a controlled pilot runbook is approved. Until then, the server must continue throwing typed vendor-access errors instead of attempting guessed calls.

## Vendor Questions

1. Can you provide official CS R4+ API documentation for appointment scheduling?
...
9. Is an on-prem connector required for UK dental practices using CS R4+?
