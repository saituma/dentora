-- Corrective migration for schema drift: clinic_history_files is recorded as
-- applied (migration 0023) in some environments (e.g. prod, journal HEAD 0029) but
-- the table is actually ABSENT — so the admin clinic-history-files feature errors
-- with "relation does not exist". Recreate it idempotently and bring it under the
-- same fail-open tenant RLS as 0030. No-op where the table already exists.
...
-- Fail-open tenant RLS, consistent with 0030.
