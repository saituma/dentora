...
export interface ConvaiClientToolConfig {
  name: string;
  description: string;
  parameters: {
    type: 'object';
...
function toolConfig(input: {
  name: string;
  description: string;
  properties?: Record<string, { type: string; description?: string }>;
  required?: string[];
}): ConvaiClientToolConfig {
  return {
    name: input.name,
...
export function buildConvaiClientTools(): ConvaiClientToolConfig[] {
  return [
    toolConfig({
      name: 'forward_call',
      description:
      ...
      properties: {
        ...
        staffName: {
          type: 'string',
          description:
            'Name or role of the staff member to forward to, such as Dr Smith or receptionist.',
        },
      },
    }),
    toolConfig({
      ...
      description:
        'Check available appointment slots for a date, optional time, or preferred part of day.',
      properties: {
        requestedDate: { type: 'string', description: 'Date to check in YYYY-MM-DD format.' },
        requestedTime: { type: 'string', description: 'Optional preferred time in HH:mm format.' },
        requestedPeriod: {
          type: 'string',
          description: 'Optional preferred period: morning, afternoon, or evening.',
        },
        appointmentDurationMinutes: {
          type: 'number',
          description: 'Appointment duration in minutes.',
        },
        maxSlots: { type: 'number', description: 'Maximum number of slots to return.' },
      },
    ...
    toolConfig({
      ...
      description:
        'Book a new appointment only after confirming the patient details and chosen slot with the caller.',
      properties: {
        ...
        phoneNumber: {
          type: 'string',
          description:
            "Patient's phone number in E.164; use the Twilio caller number when present.",
        },
        ...
        appointmentDurationMinutes: {
          type: 'number',
          description: 'Appointment duration in minutes, used only if endIso is unavailable.',
        },
    ...
    toolConfig({
      ...
      description: 'Cancel an existing appointment after verifying the caller and appointment.',
      properties: {
    ...
    toolConfig({
      ...
      description:
        'Reschedule an existing appointment after verifying the caller, appointment, and new slot.',
      properties: {
        confirmationId: { type: 'string' },
        appointmentId: { type: 'string' },
        appAppointmentId: { type: 'string' },
        ...
        appointmentDurationMinutes: {
          type: 'number',
          description: 'New appointment duration in minutes, used only if endIso is unavailable.',
        },
      },
      required: ['startIso'],
    }),
    toolConfig({
      ...
      properties: {
        phoneNumber: { type: 'string', description: "Patient's phone number in E.164." },
        dateOfBirth: { type: 'string', description: 'Date of birth in YYYY-MM-DD format.' },
      },
    ...
    toolConfig({
      ...
      description:
        'Record whether the patient agrees to SMS/WhatsApp appointment reminders. Call this only after explicitly asking them, and after the patient record exists.',
      properties: {
        phoneNumber: {
          type: 'string',
          description: "Patient's phone number in E.164; use the caller number if already known.",
        },
        consent: {
          type: 'boolean',
    ...
    toolConfig({
      ...
      description:
        'Get clinic contact details, address, staff, policies, FAQs, and practical info.',
    }),
    toolConfig({
      name: 'get_business_hours',
      description: 'Get the clinic opening hours, timezone, and closed dates.',
    }),
    toolConfig({
      name: 'acknowledge_input',
      description:
        'Acknowledge that a patient detail was captured. Use only after the caller provides name, date of birth, phone number, spelling, or reason for visit.',
      properties: {
        field: {
          type: 'string',
          description:
            'Captured field, such as fullName, dateOfBirth, phoneNumber, or reasonForVisit.',
        },
        value: { type: 'string', description: 'Captured value from the caller.' },
      },
...
// Legacy ulaw typing asset. Live calls now rely on ElevenLabs tool_call_sound
// for explicit tool execution instead of injecting audio based on assistant text.
...
async function createConvaiWebSocket(
  ...
  socket.on('open', () => {
    const initPayload = {
      ...
      client_tools: buildConvaiClientTools(),
    };
