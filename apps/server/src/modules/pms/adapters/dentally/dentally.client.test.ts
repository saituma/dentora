...
describe('DentallyClient', () => {
  ...
  it('queries documented availability, appointment reasons, practitioners, and sites endpoints', async () => {
    const fetchImpl = vi
      .fn<FetchImplementation>()
      .mockResolvedValueOnce(
        new Response(
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ sites: [{ id: 3, name: 'Site 1' }] }), {
          status: 200,
        }),
      );
    const client = new DentallyClient(authContext(), { fetchImpl });
    ...
    expect(fetchImpl.mock.calls[3]?.[0]).toBe('https://api.sandbox.dentally.co/v1/sites');
  });
