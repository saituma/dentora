...
export class CsR4PlusSimulatorSchedulingProvider implements SchedulingProviderPort {
  ...
  public async getAvailability(params: AvailabilityParams): Promise<Slot[]> {
    const startIso = `${params.requestedDate}T10:00:00.000Z`;
    const endIso = new Date(
      new Date(startIso).getTime() + params.appointmentDurationMinutes * 60_000,
    ).toISOString();
    return [{ startIso, endIso, label: `CS R4+ simulator ${startIso}` }];
  }
  ...
  public async createAppointment(params: CreateAppointmentParams): Promise<Appointment> {
    ...
    const appointment: SimAppointment = {
      id: `r4-sim-appointment-${this.sequence++}`,
      tenantId: params.tenantId,
      provider: 'cs_r4_plus',
  ...
  public async rescheduleAppointment(
    ...
    this.assertTenant(params.tenantId);
    const appointment = this.appointments.get(id);
    if (!appointment) throw new Error('CS R4+ simulator appointment not found');
  ...
  public async upsertPatient(params: UpsertPatientParams): Promise<Patient> {
    ...
    const externalId = `r4-sim-patient-${this.sequence++}`;
  ...
  public async healthCheck(): Promise<ProviderHealthCheckResult> {
    return { provider: 'cs_r4_plus', healthy: true, checkedAt: new Date().toISOString() };
  }
  ...
  private assertTenant(tenantId: string): void {
    if (tenantId !== this.tenantId) throw new Error('CS R4+ simulator tenant mismatch');
  }
}
