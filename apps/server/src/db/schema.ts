...
export const voiceToneEnum = pgEnum('voice_tone', ['calm', 'friendly', 'professional', 'urgent']);
...
export const bookingRules = pgTable('booking_rules', {
  ...
  maxFutureDays: integer('max_future_days').notNull(),
  cancellationCutoffHours: integer('cancellation_cutoff_hours').notNull(),
  doubleBookingPolicy: doubleBookingPolicyEnum('double_booking_policy').notNull(),
  emergencySlotPolicy: jsonb('emergency_slot_policy').notNull(),
  rescheduleLimits: jsonb('reschedule_limits'),
  afterHoursPolicy: jsonb('after_hours_policy').notNull(),
...
export const policies = pgTable('policies', {
  ...
  tenantId: uuid('tenant_id').notNull().references(() => tenantRegistry.id),
  configVersion: integer('config_version').notNull(),
  escalationConditions: jsonb('escalation_conditions').notNull(),
  emergencyDisclaimer: text('emergency_disclaimer').notNull(),
  sensitiveTopics: jsonb('sensitive_topics').notNull(),
  humanCallbackSlaMinutes: integer('human_callback_sla_minutes').notNull(),
...
export const voiceProfile = pgTable('voice_profile', {
  ...
  speakingSpeed: numeric('speaking_speed', { precision: 3, scale: 2 }).notNull(),
  tone: voiceToneEnum('tone').notNull(),
...
export const faqLibrary = pgTable('faq_library', {
  ...
  questionVariants: jsonb('question_variants').notNull(),
