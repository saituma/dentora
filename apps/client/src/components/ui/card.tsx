...
export function Card({
  ...
  const defaultProps = {
    className: cn(
      'relative flex flex-col rounded-2xl border bg-card not-dark:bg-clip-padding text-card-foreground shadow-xs/5 dark:backdrop-blur-md transition-[transform,box-shadow,border-color] duration-200 ease-out before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-2xl)-1px)] before:shadow-[0_1px_--theme(--color-black/4%)] dark:before:shadow-[0_-1px_--theme(--color-white/6%)]',
      className,
    ),
    'data-slot': 'card',
  };
