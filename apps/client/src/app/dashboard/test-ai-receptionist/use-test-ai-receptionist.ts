...
import {
  ...
  BARGE_IN_THRESHOLD,
  EARLY_TTS_CLAUSE_MIN_CHARS,
  EARLY_TTS_LONG_PHRASE_MIN_CHARS,
  EARLY_TTS_MIN_CHARS,
  INITIAL_MIC_DIAGNOSTICS,
  LIVE_AUDIO_RECORDER_MIME_TYPE,
  LIVE_AUDIO_RECORD_TIMESLICE_MS,
  LIVE_AUDIO_UPLOAD_MIME_TYPE,
  MAX_LIVE_AUDIO_CHUNK_BYTES,
  MIN_LIVE_AUDIO_BYTES,
  POST_ASSISTANT_AUDIO_COOLDOWN_MS,
  SILENCE_MS_BEFORE_IDLE,
  SILENCE_MS_BEFORE_SEND,
  TONE_TO_VOICE_ID,
  VAD_SILENCE_THRESHOLD,
  type ChatTurn,
  type QueuedTtsSegment,
} from './test-ai-receptionist-types';

export function useTestAiReceptionist() {
  ...
  const supportsBrowserSpeechRecognition = typeof window !== 'undefined' && !!(window.SpeechRecognition || window.webkitSpeechRecognition);
  ...
  const isLiveSocketOpen = () => liveSocketRef.current?.readyState === WebSocket.OPEN;

  const clearAudioPlayback = useCallback(() => {
    ttsQueueRef.current = [];
    browserSpeechPendingCountRef.current = 0;
    const browserResolvers = browserSpeechDrainResolversRef.current.splice(0);
    for (const resolve of browserResolvers) resolve();
    if (audioRef.current) {
  ...
  const playTtsAudioUrl = useCallback(async (audioUrl: string) => {
    try {
      ...
      const isAutoplay = error instanceof Error && error.name === 'NotAllowedError';
  ...
  const waitForTtsDrain = () => {
    if (!isProcessingTtsQueueRef.current && ttsQueueRef.current.length === 0) return Promise.resolve();
  ...
  const waitForBrowserSpeechDrain = () => {
    if (browserSpeechPendingCountRef.current === 0) return Promise.resolve();
    return new Promise<void>((resolve) => {
  ...
  const blockVoiceInputResume = (ms = POST_ASSISTANT_AUDIO_COOLDOWN_MS) => {
    ...
    if (nextAllowedAt > resumeVoiceInputAtRef.current) resumeVoiceInputAtRef.current = nextAllowedAt;
  };
  ...
  const extractSpeakableSegments = (buffer: string, flushRemainder = false) => {
    ...
    for (let index = 0; index < buffer.length; index += 1) {
      ...
      const isSoftBoundary = (char === ',' || char === ';' || char === ':') && currentLength >= EARLY_TTS_CLAUSE_MIN_CHARS;
      const isEarlyWordBoundary = char === ' ' && currentLength >= EARLY_TTS_LONG_PHRASE_MIN_CHARS;
  ...
  const processTtsQueue = useCallback(async () => {
    ...
    try {
      ...
      if (ttsQueueRef.current.length === 0) resolveTtsDrain();
    }
  }, [playTtsAudioUrl]);
  ...
  const queueTtsSegments = useCallback((segments: string[]) => {
    ...
    ttsQueueRef.current.push(...cleanedSegments.map((segment) => ({
      text: segment,
  ...
  const queueAudioUrl = useCallback((audioUrl: string) => {
    ttsQueueRef.current.push({ text: '', audioUrlPromise: Promise.resolve(audioUrl) });
    void processTtsQueue();
  }, [processTtsQueue]);
  ...
  const playConfiguredVoiceFallback = useCallback(async (text: string) => {
    ...
    if (!cleaned || configuredVoicePreviewDisabledRef.current) return false;
  ...
  const speakWithBrowserSynthesis = useCallback((text: string) => {
    ...
    const byStoredUri = browserTtsVoiceUriRef.current ? voices.find((voice) => voice.voiceURI === browserTtsVoiceUriRef.current) : undefined;
    const selectedBrowserVoice = byStoredUri
      ...
      ?? voices[0];
    if (selectedBrowserVoice?.voiceURI) browserTtsVoiceUriRef.current = selectedBrowserVoice.voiceURI;
  ...
  const normalizeCallTranscript = (value: string) => value.toLowerCase().replace(/[^\w\s]/g, ' ').replace(/\s+/g, ' ').trim();
  ...
  const isLikelyAssistantEcho = (transcript: string): boolean => {
    ...
    if (Date.now() - lastAssistantUtteranceAtRef.current >= ASSISTANT_ECHO_WINDOW_MS) return false;
    ...
    if (normalized.length >= 24 && (normalized.includes(lastAssistant) || lastAssistant.includes(normalized))) return true;
    ...
    let overlapCount = 0;
    for (const token of transcriptTokens) if (assistantTokens.has(token)) overlapCount += 1;
    if (minTokenCount >= 3 && overlapCount / minTokenCount >= 0.7) return true;
    if (transcriptWordList.length >= 4) {
      ...
      for (let index = 0; index <= transcriptWordList.length - 4; index += 1) {
        ...
        if (assistantTextWithPadding.includes(` ${phrase} `)) return true;
      }
    }
    return false;
  };
  ...
  const startFallbackRecorder = async () => {
    ...
    try {
      ...
      const sendBufferedUtterance = async () => {
        ...
        const utteranceBlob = new Blob(utteranceAudioChunksRef.current, { type: LIVE_AUDIO_UPLOAD_MIME_TYPE });
        utteranceAudioChunksRef.current = [];
        utteranceAudioBytesRef.current = 0;
        setInterimTranscript('Transcribing...');
        setIsListening(false);
        ...
        try {
          ...
          if (transcriptionSessionId !== transcriptionSessionRef.current || aiRespondingRef.current || !isCallActiveRef.current) return;
      ...
      const startNewRecorder = () => {
        ...
        const recorder = new MediaRecorder(streamForRecorderRef.current, { mimeType: LIVE_AUDIO_RECORDER_MIME_TYPE });
        recorder.ondataavailable = (event) => {
          if (!isCallActiveRef.current || !event.data || event.data.size === 0) return;
        ...
        recorder.start(LIVE_AUDIO_RECORD_TIMESLICE_MS);
      };
  ...
  const openLiveSocket = async (): Promise<boolean> => {
    ...
    if (liveVoiceOverride?.voiceId) socketUrl.searchParams.set('overrideVoiceId', liveVoiceOverride.voiceId);
    ...
    return new Promise<boolean>((resolve) => {
      ...
      socket.onmessage = (event) => {
        ...
        switch (message.event) {
          ...
          case 'transcript_final': {
            ...
            const normalized = normalizeCallTranscript(transcript);
            const duplicate = normalized && normalized === lastLiveTranscriptRef.current && Date.now() - lastLiveTranscriptAtRef.current < 2500;
            if (duplicate) return;
          ...
          case 'assistant_turn_start':
            ...
            setTurns((prev) => [...prev, { speaker: 'receptionist', text: '', ts: currentAssistantTurnTsRef.current! }]);
            stopSpeechRecognition();
            stopFallbackRecorder();
            return;
      ...
      socket.onclose = () => {
        ...
        if (isCallActiveRef.current) setIsListening(false);
      };
    });
  };

  const startSpeechRecognition = useCallback(async (): Promise<boolean> => {
    if (!supportsBrowserSpeechRecognition || !isCallActiveRef.current || aiRespondingRef.current) return false;
    ...
    recognition.onresult = (event) => {
      const inAssistantPlaybackWindow = aiRespondingRef.current || browserSpeechPendingCountRef.current > 0 || Date.now() < resumeVoiceInputAtRef.current;
      ...
      if (isLiveSocketOpen() && !aiRespondingRef.current) {
        liveSocketRef.current?.send(JSON.stringify({ event: 'user_text', transcript: cleanedFinal }));
      }
    };
    ...
    recognition.onend = () => {
      speechRecognitionRef.current = null;
      setIsListening(false);
      if (!shouldRestartSpeechRecognitionRef.current || !isCallActiveRef.current || aiRespondingRef.current) return;
      clearSpeechRecognitionRestartTimer();
  ...
  }, [canUseUploadFallback, clearAudioPlayback, playConfiguredVoiceFallback, speakWithBrowserSynthesis, supportsBrowserSpeechRecognition]);
  ...
  const startLiveInput = useCallback(async (enableVoiceInput = true) => {
    ...
    if (!enableVoiceInput) return true;
    if (canUseUploadFallback && !supportsBrowserSpeechRecognition) await startFallbackRecorder();
    if (supportsBrowserSpeechRecognition) await startSpeechRecognition();
    return true;
  ...
  const sendCallerUtterance = async (utterance: string) => {
    ...
    if (!cleaned || aiRespondingRef.current) return;
    aiRespondingRef.current = true;
    setIsStreamingResponse(true);
    stopSpeechRecognition();
    stopFallbackRecorder();
    ...
    setTurns((prev) => [...prev, callerTurn, { speaker: 'receptionist', text: '', ts: receptionistTs }]);
    try {
      ...
      if (!responseText) throw new Error('AI receptionist returned an empty response');
      const { segments, remainder } = extractSpeakableSegments(ttsAccumulatorRef.current, true);
      ttsAccumulatorRef.current = remainder;
      queueTtsSegments(segments.length > 0 ? segments : (responseText.trim() ? [responseText.trim()] : []));
      await waitForTtsDrain();
      await waitForBrowserSpeechDrain();
      blockVoiceInputResume();
      await waitForVoiceInputResumeWindow();
      ...
      if (isCallActiveRef.current) {
        window.setTimeout(() => {
          if (isCallActiveRef.current && !aiRespondingRef.current) void startLiveInput();
        }, 120);
      }
    }
  };
  ...
  const sendManualMessage = async () => {
    ...
    setManualInput('');
    if (isLiveSocketOpen()) {
      liveSocketRef.current?.send(JSON.stringify({ event: 'user_text', transcript: text }));
      return;
    }
...
export type UseTestAiReceptionistResult = ReturnType<typeof useTestAiReceptionist>;
